...
class CIMYIEElementsCHPURizator extends CIMYIECHPURizator
{
	...
	function UpdateCodes( $StartID, $IBLOCK_ID, $arSettings )
	{
		...
		$time_start = time();
		$arBadID = array();
		$query_count = "select count(ID) from b_iblock_element WHERE IBLOCK_ID = '".$IBLOCK_ID."' and ID >= '0'";
		...
		{
			$LastID = $row["ID"];
			$NEW_CODE = CIMYIEElementsCHPURizator::_GenerateCode( $LastID, $row["NAME"] );
			$result_upd = CIMYIEElementsCHPURizator::_UpdateCodeByID( $LastID, $NEW_CODE );
			if(!$result_upd)
				$arBadID[] = $LastID;
			$time_now = time();
		...
		$result_html = CIMYIEElementsCHPURizator::_ReturneJS( $LastID, $IBLOCK_ID, $arSettings, $full_count_el, $chetchik, $the_end, $time_start, $arBadID );
		return $result_html;
	}
	...
	{
		global $DB, $APPLICATION;
		$query_checking = "SELECT count(ID) FROM b_iblock_element
						WHERE `CODE` = '".$CODE."'
						LIMIT 1";
	...
	function _ReturneJS( $LastID, $IBLOCK_ID, $arSettings, $full_count_el, $chetchik, $the_end, $time_start, $arBadID )
	{
		...
		$query_covered = "select count(ID) from b_iblock_element WHERE IBLOCK_ID = '".$IBLOCK_ID."' and ID <= '".$LastID."'";
		...
		$query_balance = "select count(ID) from b_iblock_element WHERE IBLOCK_ID = '".$IBLOCK_ID."' and ID > '".$LastID."'";
		if($arSettings["only_empty_code"]=="Y")
...
$return.= CIMYIEElementsCHPURizator::_ReturneJSBadID( $arBadID );
