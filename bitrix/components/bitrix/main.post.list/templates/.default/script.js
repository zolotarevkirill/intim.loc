;(function(window){
	...
	window.FCList = function (params, add) {
		...
		this.DATE_TIME_FORMAT = (!!params["params"]["DATE_TIME_FORMAT"] ? params["params"]["DATE_TIME_FORMAT"] : null);
 		...
 		add : function(id, data, edit, animation) {
			...
			var
				...
				html = (!!data["message"] ? data["message"] :  fcParseTemplate(
					...
					{ DATE_TIME_FORMAT : this.DATE_TIME_FORMAT, RIGHTS : this.rights } )),
	...
	window["fcParseTemplate"] = function(data, params)
	{
		...
		if (!!res && !!data["messageFields"])
		{
			...
			if (!!res["POST_TIMESTAMP"])
			{
				...
				replacement["DATE"] = BX.date.format(dateFormat, res["POST_TIMESTAMP"], false, true);
			}
			if (!!params["RIGHTS"])
			{
